...
export const fetchFakeflixMoviesRquest = () => ({
  type: moviesActionTypes.FETCH_FAKE_FLIX_MOVIES_REQUEST,
});

export const fetchFakeflixMoviesSuccess = (payload) => ({
  type: moviesActionTypes.FETCH_FAKE_FLIX_MOVIES_SUCCESS,
  payload,
});
export const fetchFakeflixMoviesFailure = (error) => ({
  type: moviesActionTypes.FETCH_FAKE_FLIX_MOVIES_FAILURE,
  payload: error,
});

// eslint-disable-next-line no-unused-vars
export const fetchFakeflixMoviesAsync = (fetchUrl, isPage = false) => {
  return (dispatch, getState) => {
    dispatch(fetchFakeflixMoviesRquest);
    const authToken = getState().auth.currentUser.tokens.access.token;
    axios
      .get(fetchUrl, {
        headers: {
      ...
      .then((res) => {
        const fakeflixMovies = res.data.map((el) => ({
          ...el,
          isFavourite: false,
        }));
        dispatch(fetchFakeflixMoviesSuccess(fakeflixMovies));
      })
      .catch((error) => {
        const errorMessage = error.message;
        dispatch(fetchFakeflixMoviesFailure(errorMessage));
      });
  };
};
